...
class GreatCircleTestCase(activity: Activity) : MaplyTestCase(activity) {
    ...
    fun buildGreatCircle(control: BaseController, a: Point2d, b: Point2d, color: Int): ComponentObject {
        ...
        val subDivObj: VectorObject = if (control is GlobeController)
            vecObj.subdivideToGlobeGreatCircle(0.001)
        else
            vecObj.subdivideToFlatGreatCircle(0.001)
